...
const ItemListContainer = ({ addToCart }) => {
  ...
  const [addedToCart, setAddedToCart] = useState({}); // Define addedToCart
  ...
  const filteredProducts = productsData.filter(product => {
    return (!animal || product.animal.toLowerCase() === animal) &&
           (!tipoProducto || product.tipoProducto.toLowerCase() === tipoProducto);
  });

  const handleAddToCart = (id) => {
    setAddedToCart(prevState => ({ ...prevState, [id]: true }));
    setTimeout(() => setAddedToCart(prevState => ({ ...prevState, [id]: false })), 3000); // reset after 3 seconds
  };
